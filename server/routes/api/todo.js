...
router.get("/", auth.verifyToken, (req, res, next) => {
  Todo.find((err, todos) => {
    if (err) {
      res.send({
        error: { message: "Data cannot be fetched..Please try again" },
      });
    } else {
...
router.post("/add", auth.verifyToken, (req, res, next) => {
  if (!req.body.body) {
    ...
    todo.save((err, added) => {
      if (err) {
        res.send({
          error: { message: "Task cannot be added.Please try again!!" },
        });
      } else {
...
router.put("/update", auth.verifyToken, (req, res, next) => {
  ...
  if (!id) {
    ...
    res.send({
      error: {
        message: "Todo can't be updated due to providing insufficient info.",
      },
    });
  } else {
    if (body) {
    ...
    Todo.findByIdAndUpdate(
      ...
      (err, updated) => {
        if (err) {
          res.send({
            error: { message: "Task cannot be updated.Please try again!!" },
          });
        } else {
...
router.delete("/delete", auth.verifyToken, (req, res, next) => {
  if (!req.body.id) {
    ...
    Todo.findByIdAndDelete(req.body.id, (err, deleted) => {
      if (err) {
        res.send({
          error: { message: "Task cannot be deleted.Please try again!!" },
        });
      } else {
